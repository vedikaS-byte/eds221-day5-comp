...
# The line below... time_vector
time_yr <- seq(from = 0, to = 8, by = 1)

# The line below... store pop sizes
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

# The line below... first row populated with first year pop
proj_year[1, ] <- fish_year0

# The for loop below... for every iteration from the second position (since first year/row populated) to the end of the time_yr vector, project the yr based on the leslie*population of first row of matrix. Estimate pop size for each yr based on pop size of the previous yr
...
# update column names for pop stages
